...
# Open the tsv file and iterate over each word
with open('word_search.tsv') as datafile: 					#opening the tab sepearted value file
	for row in datafile:
		word, frequency = row.split('\t') 			#splitting into word and frequency
		word_count[word] = int(frequency.strip())	#storing the freq of each word into wordcount dictionary
		words.append(word)							#storing each word into the list 'words'
...
#This sorts the words based on a match with the search keyword.
